...
class TeamCityLoggerOld
{
    ...
    public function testFinished($name, $duration = null)
    {
        $this->write('testFinished', [
            'name'     => $name,
            'duration' => $duration > 0 ? round($duration * 10000000) : null,
        ]);
    }
     ...
     * @param string      $name     The test name.
     * @param string      $message  The textual representation of the error.
     * @param string|null $details  The information on the test failure,
     *                              typically a message and an exception stacktrace.
     * @param string|null $actual   The actual value.
